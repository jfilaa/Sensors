...
LPS25HB_Error_et LPS25HB_Get_RawPressure(int32_t *raw_press)
{
  ...
  if(LPS25HB_ReadReg(LPS25HB_PRESS_OUT_XL_REG, 3, buffer)) return LPS25HB_ERROR;
...
{  
  ...
  if(LPS25HB_ReadReg(LPS25HB_STATUS_REG, 1, &tmp)) return LPS25HB_ERROR;
...
{  
  ...
  if(LPS25HB_Get_Temperature(&Tout)) return LPS25HB_ERROR;
  
  Measurement_Value->Tout=Tout;
  
  if(LPS25HB_Get_Pressure(&Pout)) return LPS25HB_ERROR;
...
{    
  ...
  if(LPS25HB_Set_PowerDownMode(LPS25HB_RESET)) return LPS25HB_ERROR;
  
  /* Make LPS25HB Reset and Reboot */ 
  if(LPS25HB_SwResetAndMemoryBoot()) return LPS25HB_ERROR;
 
  /* Dump of data output */
  if(LPS25HB_Get_Measurement(& Measurement_Value)) return LPS25HB_ERROR;
